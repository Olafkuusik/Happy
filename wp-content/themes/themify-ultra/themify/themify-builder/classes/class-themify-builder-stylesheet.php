...
class Themify_Builder_Stylesheet {
	...
	public function get_rules( $component_name, $settings ) {
		...
		if ( isset( Themify_Builder_Model::$modules[ $component_name ] ) ) {
			$styling = Themify_Builder_model::$modules[$component_name]->get_styling();
	 ...
	 * @return string
	 */
	private function get_gradient_value( $string ) {
		$string = trim( str_replace( 'background-image:', '', $string ) );
			...
			if ($option['type'] === 'multi') {
				...
				switch ( $option['id'] ) {
					case 'background_gradient':
						if ( 'gradient' === $settings['background_type'] && isset( $settings[$option['id'] . '-css'] ) ) {
							$new = array(
								'id' => $option['id'],
					...
					case 'cover_gradient':
						if ( 'cover_gradient' === $settings['cover_color-type'] && isset( $settings[$option['id'] . '-css'] ) ) {
							$new = array(
								'id' => $option['id'],
								'value' => $settings[$option['id'] . '-css'],
								'prop' => 'background-image',
					...
					case 'cover_gradient_hover':
						if ( 'hover_gradient' === $settings['cover_color_hover-type'] && isset( $settings[$option['id'] . '-css'] ) ) {
							$new = array(
								'id' => $option['id'],
								'value' => $settings[$option['id'] . '-css'],
								'prop' => 'background-image',
				...
				if( isset($settings[$option['id'] . '-type'], $settings[$option['id'] . '-css']) && $settings[$option['id'] . '-type'] === 'gradient'  ) {
					$new = array(
						'id' => $option['id'],
						'value' => $settings[$option['id'] . '-css'],
						'prop' => 'background-image',
