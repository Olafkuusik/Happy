...
class Themify_Popup {
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_themify_popup_save_cookie', array( &$this, 'save_cookie' ) );
		add_action( 'wp_ajax_nopriv_themify_popup_save_cookie', array( &$this, 'save_cookie' ) );
		add_action( 'wp_ajax_themify_popup_update_page_view', array( &$this, 'update_page_view' ) );
		add_action( 'wp_ajax_nopriv_themify_popup_update_page_view', array( &$this, 'update_page_view' ) );
	...
	public function hooks() {
		if( ! is_singular( 'themify_popup' ) ) {
			...
			add_action( 'wp_footer', array( $this, 'loader' ), 11 );
	...
	function save_cookie() {
		if( isset( $_POST['popup_id'] ) ) {
			$id = $_POST['popup_id'];
			$days = $_POST['expiration_days'];
			setcookie( "themify-popup-{$id}", 1, time() + 3600 * 24 * $days, COOKIEPATH, COOKIE_DOMAIN, false );
		}
		die();
	}

	function update_page_view() {
		if( isset( $_POST['action'] ) && $_POST['action'] == 'themify_popup_update_page_view' ) {
			$days = 1;
	...
	function is_popup_visible( $id ) {
		...
		if( themify_popup_check( 'popup_show_once' ) && isset( $_COOKIE["themify-popup-{$id}"] ) ) {
			$visible = false;
		}
