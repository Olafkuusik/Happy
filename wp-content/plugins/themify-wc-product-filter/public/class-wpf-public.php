...
class WPF_Public {
    ...
    public function __construct($plugin_name, $version) {
        ...
        if (!defined('DOING_AJAX') || !DOING_AJAX) {
            add_action('wp_head', array($this, 'result_page'));
        ...
        add_filter('widget_text', array($this, 'get_shortcode'), 10, 1);
    ...
    public function result_container($content = '') {
        ...
        if (is_woocommerce()) {
            echo '<div data-slug="' . $slug . '" class="wpf-search-container">',self::$result;
        } else {
    ...
    public function get_result(array $data, array $form) {
        ...
        return '<div class="wpf-search-wait"></div>' . $result;
    }
    ...
    public function parse_query(array $post, array $form, $build = true) {
        ...
        if ($build) {
            ...
            if (!empty($data['posts_per_page'])) {
                $query_args['posts_per_page'] = intval($data['posts_per_page']);
            }
